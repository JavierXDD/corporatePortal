...
<html>
  	...
  	$(function(){
  			$("table tr td").find("a:first").click(function(){
  				if(window.confirm("确定删除?")){
  					...
  					$.ajax({
  						url:"/LiTian/AboutServlet",
  						data:{method:"ajaxDelete",aid:aId},
  						type:"POST",
  						dataType:"text",
  						async:false,
  						cache:false,
  ...
  <div class="container clearfix" style="margin-left:-191px">
    ...
    <div class="crumb-wrap">
			...
			<div class="result-wrap">
                <div class="result-title">
                    <div class="result-list">
                        <a href="${pageContext.request.contextPath }/manager/about/addAbout.jsp"><i class="icon-font"></i> 增加项目</a>
                    </div>
                </div>
        	...
        	<tbody >
        <c:forEach items="${pb.beanList}" var="aboutlist">
        	<tr class="${aboutlist.aId} abc">
        		<td>${aboutlist.aEditors.uName }</td>
        		<td>${aboutlist.aTitle }</td>
        		<td>${aboutlist.aTime }</td>
				<td><a id="${aboutlist.aId}" style="cursor:pointer;" >删除</a>  <a href='<c:url value="AboutServlet?method=editShowAbout&aId=${aboutlist.aId }"></c:url>'>编辑</a></td>
        	</tr>	 	
      	 </c:forEach>
        </tbody>
        </table>
