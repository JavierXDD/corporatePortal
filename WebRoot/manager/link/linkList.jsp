  ...
  <div class="container clearfix" style="margin-left:-191px">
    <div class="main-wrap">
    	<div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i>链接管理</div>
        </div>
        	...
        	<tbody >
        <c:forEach items="${pb.beanList}" var="linkList">
        	<tr class="${linkList.iId} abc">
        		<td>${linkList.iName}</td>
        		<td>${linkList.iValue }</td>
				<td>
				<a id="${linkList.iId}" style="cursor:pointer;" href='<c:url value="LinkServlet?method=deleteLink&iId=${linkList.iId }"></c:url>'>删除</a> 
				<a href='<c:url value="LinkServlet?method=findLinkByiId&iId=${linkList.iId }"></c:url>'>编辑</a>
				</td>
        	</tr>	 	
      	 </c:forEach>
        </tbody>
