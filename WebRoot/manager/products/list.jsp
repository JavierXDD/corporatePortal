  	...
  	$(function(){
  			$("table tr td").find("a:first").click(function(){
  				if(window.confirm("确定删除?")){
  					...
  					$.ajax({
  						url:"/LiTian/ProductsServlet",
  						data:{method:"ajaxDelete",pId:pid},
  						type:"POST",
  						dataType:"text",
  						async:false,
  						cache:false,
  ...
  <div class="container clearfix" style="margin-left:-191px">
    <div class="main-wrap">
    	<div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i>产品列表</div>
        </div>
			<div class="result-wrap">
                <div class="result-title">
                    <div class="result-list">
                        <a href="${pageContext.request.contextPath }/manager/products/addProducts.jsp"><i class="icon-font"></i> 增加新产品</a>
                    </div>
                </div>
        	...
        	<tbody >
        <c:forEach items="${pb.beanList}" var="productslist">
        	<tr class="${productslist.pId } abc">
        		<td>${productslist.pTypes.cName }</td>
        		<td>${productslist.pName }</td>
				<td>
				<a id="${productslist.pId}" style="cursor:pointer;" >删除</a>&nbsp;&nbsp;
			 	<a href='<c:url value="/ProductsServlet?method=editShowProduct&pid=${productslist.pId }"></c:url>'>编辑</a>
				</td>
        	</tr>	 	
      	 </c:forEach>
        </tbody>
